-- Simple Database Schema (Fixed Version - No Duplicates)
...
-- Create simple users table (no auth.users dependency)
CREATE TABLE users (
...
-- No RLS needed - we'll handle permissions in the application
...
('admin@example.com', 'admin', '$2b$10$OaJaCzXOSHt5OK7NrFi9qu/vYJbqYysnhiBUqQiUVC0kKvlt18.W6', 'admin')
ON CONFLICT (email) DO NOTHING;

-- Insert default system settings (using ON CONFLICT to avoid duplicates)
INSERT INTO system_settings (key, value, updated_by) 
SELECT 'system_prompt', 'You are a helpful AI assistant with access to a MySQL database tool for querying auto parts data and a delivery tracking tool.
...
DO $$
BEGIN
    RAISE NOTICE 'Database setup complete! Default admin: admin@example.com / admin123';
END $$; 